...
var data_json = require('./new_data_IMP.json');
...
const TD_contract_json2 = require('/Users/sid_mukkamala/Documents/Thesis2/code/truffleprojects/truffleanew/build/contracts/TruthDiscovery2.json');
...
const weightupdate = async () => {
	...
	var pt_value = partial_truth(Workers,data, data2)
	// var pt_value1 = partial_truth(Workers, data2)

	var partial_truth_scaled =  Math.round(pt_value[0]*10000)
	var partial_truth_scaled2 =  Math.round(pt_value[1]*10000)
	var weights_scaled = Math.round(pt_value[2])
	console.log("PTval 1 is : ", pt_value)
	// console.log("PTval 2 is : ", pt_value)
	// console.log("SCALED values are: ", partial_truth_scaled,weights_scaled)
...
const get_finaltruth = async () => {
	...
	console.log("Value 2 got out is: ",value2);

	// Using final_truth, update the weights
	var new_workers =  worker_update(Workers,value, value2);
...
const resetvals = async () => {
	...
	TD_contract2.methods.reset().send({from:user1})
}
// THIS IS THE ONE
...
function partial_truth(workers_lst, data_lst, data_lst2){
	let pt = 0;
	let pt1 = 0;
	let weights = 0;
...
function worker_update(worker_lst, final_truth, final_truth2){
// If the worker provided a value at this instance, then update the N count, after applying decay
// console.log("Worker update is beginning")

for (i = 0; i < N; i++) {
	temp_worker = worker_lst[i]


	worker_lst[i].N = 2 + temp_worker.N*decay
	worker_lst[i].Err = ((final_truth - temp_worker.val[0])**2) + ((final_truth - temp_worker.val[1])**2) + temp_worker.Err*decay
	worker_lst[i].val = [NaN,NaN]
...
set_up();
// weightupdate();
var Id = setInterval(scehduler, 9000);
// weightupdate();

...
const TD_contract_json3 = require('/Users/sid_mukkamala/Documents/Thesis2/code/truffleprojects/truffleanew/build/contracts/TruthDiscovery3.json');
...
const weightupdate = async () => {
	...
	var pt_value = partial_truth(Workers,data, data2,data3)
	...
	var weights_scaled = Math.round(pt_value[3])
	// console.log("The submission values are: ", pt_value)
	// console.log("SCALED values are: ", partial_truth_scaled,weights_scaled)
...
const get_finaltruth = async () => {
	...
	// Using final_truth, update the weights
	worker_update(Workers,value, value2, value3);
	// console.log(Workers)
...
const resetvals = async () => {
	...
	TD_contract3.methods.reset().send({from:user1})
}
// THIS IS THE ONE
...
function partial_truth(workers_lst, data_lst, data_lst2, data_lst3){
	let pt = 0;
	let pt1 = 0;
	let pt2 = 0;
	let weights = 0;
...
function worker_update(worker_lst, final_truth, final_truth2, final_truth3){
...
for (i = 0; i < N; i++) {
	...
	worker_lst[i].Err = ((final_truth - temp_worker.val[0])**2) + ((final_truth2 - temp_worker.val[1])**2) + ((final_truth3 - temp_worker.val[2])**2) + temp_worker.Err*decay
	worker_lst[i].val = [NaN,NaN,NaN]
